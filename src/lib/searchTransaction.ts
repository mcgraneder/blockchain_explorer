...
export const searchTransaction = async (
  ...
  if (!transaction.queryTx) {
    transaction.queryTx = await queryMintOrBurn(renJS.provider, transaction.txHash, getChain);
  }
...
export const getTransactionDepositInstance = async (
  ...
  if (!summary.fromChain) {
    throw new Error(`Fetching transaction details not supported yet for ${summary.fromLabel}.`);
  }

  if (!summary.toChain) {
    throw new Error(`Fetching transaction details not supported yet for ${summary.toLabel}.`);
  }
  ...
  const txParams: TransactionParams = {
    ...
    fromTx: {
      ...
      txid,
      explorerLink:
        (summary.fromChain && summary.fromChain.transactionExplorerLink({ txid })) || "",
  ...
  if (deposit.hash !== searchDetails.hash) {
    console.group(`Expected ${deposit.hash} to equal ${searchDetails.hash}.`);
