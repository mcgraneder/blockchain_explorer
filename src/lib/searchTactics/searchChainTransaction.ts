...
const queryTxsByTxid = async (
  ...
  const response: { txs: Array<RPCResponses[RPCMethod.QueryTx]["tx"]> } =
    await provider.sendMessage("ren_queryTxsByTxid" as any, { txid: utils.toURLBase64(txid) }, 1);
...
export const searchChainTransaction: SearchTactic<RenVMTransaction> = {
  match: (searchString: string, getChain: (chainName: string) => Chain | null) =>
    allChains
      .map((chain) => getChain(chain.chain))
      .map((chain) =>
        utils.doesntError(() =>
          chain ? chain.validateTransaction({ txHash: searchString }) : false
        )()
      )
      .reduce(OR, false),
  search: async (
          ...
          .map((chain) => {
            try {
              return chain &&
                ...
                ? utils.toURLBase64(chain.txHashToBytes(searchString))
                : null;
            } catch (error) {}
            return null;
          })
    ...
    if (formats.length) {
      updateStatus(
        `Looking up ${formats.length} chain transaction format${formats.length === 1 ? "" : "s"}...`
      );
    } else {
      return [];
    }
    ...
    for (const format of formats) {
      try {
        queryTxs = await queryTxsByTxid(renJS.provider, format!, getChain);
        break;
      } catch (error: any) {
        continue;
      }
