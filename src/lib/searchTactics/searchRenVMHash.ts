...
const getChainLabel = (chain: string, network: RenNetwork): { short: string; full: string } => {
...
export const summarizeTransaction = async (
  ...
  let inTx:
    ...
    | undefined;
  if (fromChain && searchDetails.in?.txid && searchDetails.in?.txid.length > 0) {
  ...
  if (asset === "gETH") {
    ...
    !assetOriginChain &&
    (Object.values(Ethereum.assets[RenNetwork.Mainnet]) as string[]).includes(asset)
  ) {
  ...
  const { full: fromLabel, short: fromLabelShort } = getChainLabel(from, NETWORK);
...
export const searchRenVMHash: SearchTactic<RenVMTransaction> = {
  ...
  search: async (
    ...
    let queryTx = await queryMintOrBurn(renJS.provider, searchString, getChain);
