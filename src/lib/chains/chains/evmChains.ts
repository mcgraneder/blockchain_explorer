...
export const EthereumDetails: ChainDetails<Ethereum> = {
  ...
  getOutputParams: async (
    ...
    getEthereumMintParams(mintChain as EthereumBaseChain, to, payload, asset),
};
...
export const FantomDetails: ChainDetails<Fantom> = {
  ...
  getOutputParams: async (
    ...
    payload: string,
    asset: string
  ): Promise<EVMPayloadInterface> => getEthereumMintParams(mintChain as Fantom, to, payload, asset),
};
...
export const GoerliDetails: ChainDetails<Goerli> = {
  ...
  getOutputParams: async (
    ...
    payload: string,
    asset: string
  ): Promise<EVMPayloadInterface> => getEthereumMintParams(mintChain as Goerli, to, payload, asset),
};
...
export const CatalogDetails: ChainDetails<Catalog> = {
  ...
  type: ChainType.EVMChain,
  etherscanApi: {},
  usePublicProvider: (network: RenNetwork) => getPublicEthereumProvider<Catalog>(Catalog, network),
...
export const KavaDetails: ChainDetails<Kava> = {
  ...
  usePublicProvider: (network: RenNetwork) => getPublicEthereumProvider<Kava>(Kava, network),
  ...
  getOutputParams: async (
    ...
    payload: string,
    asset: string
  ): Promise<EVMPayloadInterface> => getEthereumMintParams(mintChain as Kava, to, payload, asset),
};
...
const getPublicEthereumProvider = <T extends EthereumBaseChain>(
  ...
  const provider = new StaticJsonRpcProvider(urls[0], parseInt(config.config.chainId, 16));
...
const getEthereumMintParams = async (
  ...
  if (
    utils.Ox(to.toLowerCase()) === "0xa9975b1c616b4841a1cc299bc6f684b4d1e23a61" ||
    utils.Ox(to.toLowerCase()) === "0x82f2e6c57b7fde0ebb42f22bfc6b4a297e7d9be8" ||
    ...
    parameters = [
      ...
      },
      {
        name: "mintToken",
        type: "address",
        value: await (mintChain as EthereumBaseChain).getMintAsset(asset),
        notInPayload: true,
      },
      {
        name: "burnToken",
      ...
      },
      {
        ...
        value: [utils.Ox("00".repeat(20)), 0, utils.Ox("00".repeat(20)), Buffer.from([])],
        notInPayload: true,
      },
    ];
  } else {
...
const getABIFromEtherscan: ABITactic = async (
  ...
  if (evmChains[chain.chain] && evmChains[chain.chain].etherscanApi?.[NETWORK]) {
 ...
 */
export const getEvmABI = async (chain: ChainCommon, to: any): Promise<AbiItem[]> => {
