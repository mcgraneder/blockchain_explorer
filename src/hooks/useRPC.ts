...
import { IChain as Chain } from "src/types/chains.d.";
...
function useEthRPC(queryUrlOverride: string): [ERPC, Dispatch<Chain>] {
  ...
  const [urlOverride] = useState("https://rpc.catalog.fi/testnet");
  // const [urlOverride] = useState(queryUrlOverride);

  useEffect(() => {
    if (selectedChain === undefined && !urlOverride) {
      return;
    }
