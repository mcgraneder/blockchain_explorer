...
export const AmountWithPrice = ({
  ...
  const amountInUsd = useMemo(() => {
    return amount && tokenPrice
      ? new BigNumber(amount).times(tokenPrice).decimalPlaces(2).toFormat()
      : undefined;
  }, [tokenPrice, amount]);
    ...
    <span>
      <>
        <span>
          ...
          <span className='text-gray-400'>{amountFixedInsignificant}</span>{" "}
        </span>
        {assetLabel}
        <span className='text-gray-500'>{amountInUsd ? <> (US${amountInUsd})</> : null}</span>
      </>
    </span>
  );
};
